...
trans <- data_transfer %>% 
  count(Year) %>% 
  rename(Total_Transfer = n)

#combine raw data with transfer data
merge <- merge(x=mytable,y=trans,by="Year",all.x=TRUE)  %>% 
  mutate(Total_Transfer=replace_na(Total_Transfer,0)) %>% 
  mutate(Net_Sample = ifelse(Year == Year[1], 
                             Total_Sample,Total_Sample-dplyr::lag(Total_Transfer))) %>% 
  mutate(Cumulative_Sample = cumsum(Net_Sample))

#calculate percent of total tranหfer by year
final_table <- merge %>%  
  mutate(Percent_Transfer =Total_Transfer/Cumulative_Sample*100 ) 
